...
Guides are outcome-driven, ordered workflows rather than long-form articles. A `guide` requires `outcome` and a unique ordered `guideSteps` array. Each step has a stable lowercase-hyphenated ID, title, description, and optional estimated minutes. `prerequisites` and `guideResources` provide overview context. Step bodies use `guide-step` documents with `guideSlug`, `guideStepId`, and `guideStepOrder`, and resolve to `/guides/[slug]/[step]`.
